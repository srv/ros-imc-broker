...
#ifndef ROS_IMC_BROKER_VEHICLE_ADAPTER_HPP_INCLUDED_
#define ROS_IMC_BROKER_VEHICLE_ADAPTER_HPP_INCLUDED_
...
namespace ros_imc_broker
{
  ...
  {
  public:
    VehicleAdapter(ros::NodeHandle& node_handle):
      nh_(node_handle),
      tcp_client_(NULL),
      tcp_client_thread_(NULL)
    {
      srv_.setCallback(boost::bind(&VehicleAdapter::onReconfigure, this, _1, _2));
      advertiseAll();
      subscribeAll();
    }
  ...
  private:
    ...
    {
      stop();
      tcp_client_ = new TcpLink(boost::bind(&VehicleAdapter::sendToRosBus, this, _1));
    ...
    sendToTcpServer(const T& msg)
    {
      ...
      //msg.setSource(0x0001);
      //udp_client_->send(&msg);


      IMC::Announce t_msg;
      t_msg.setSource(0x5001);
      t_msg.sys_name = "Bogus";
      t_msg.sys_type = 0;
      udp_client_->send(&t_msg, "10.0.2.90", 6001);
...
#define MESSAGE(id, abbrev, md5)                                        \
        ...
        {                                                               \
          ...
          ros::Subscriber sub = nh_.subscribe(topic, 1000, &VehicleAdapter::sendToTcpServer<IMC::abbrev>, this); \
